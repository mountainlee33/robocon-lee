...
void Motor_Init(void)
{
	...
	M2006instrin.GEARRATIO = 5;	   //6.117f;    //外参齿数比
	...
	motorlimit.zeroCurrent = 1800;
	motorlimit.zeroSP = -200;
	motorlimit.Motorstuck = 0;//打开堵转检测
	motorlimit.Motortimeout = 1;//打开超时检测
	motorlimit.openzero = 1;	//打開自動尋靈
	...
	motor[1].begin = true;
	motor[1].mode = zero; //尋靈模式
	motor[1].valueSet.current = 50;
	motor[1].valueSet.angle = 0;
	motor[1].valueSet.velocity = 100;
	motor[1].limit = motorlimit;
	PID_Init(&motor[1].PIDs, 8.0, 0.5, 0, 1, motor[1].valueSet.velocity);//6,0.4//8,0.6
	PID_Init(&motor[1].PIDx, 1.3, 0.15, 0.0, 1, motor[1].valueSet.pulse);//6,0.15//35,0.38,0.31

	/*M2006（3）电机初始化*/
	motor[2].intrinsic = M2006instrin;
	motor[2].enable = DISABLE;
	motor[2].begin = true;
	motor[2].mode = zero; //尋靈模式
	motor[2].valueSet.current = 50;
	motor[2].valueSet.angle = 0;
	motor[2].valueSet.velocity = 100;
	motor[2].limit = motorlimit;
	PID_Init(&motor[2].PIDs, 8.0, 0.5, 0, 1, motor[2].valueSet.velocity);
	PID_Init(&motor[2].PIDx, 1.3, 0.15, 0.0, 1, motor[2].valueSet.pulse);

	/*M2006（4）电机初始化*/
	motor[3].intrinsic = M2006instrin;
	motor[3].enable = DISABLE;
	motor[3].begin = true;
	motor[3].mode = zero; //尋靈模式
	motor[3].valueSet.current = 50;
	motor[3].valueSet.angle = 0;
	motor[3].valueSet.velocity = 1000;
	motor[3].limit = motorlimit;//3.5, 0.12, 0, 0.4;5, 0.3, 0.2, 1
	PID_Init(&motor[3].PIDs,8.0, 0.5, 0, 1, motor[3].valueSet.velocity);
	PID_Init(&motor[3].PIDx,1.3, 0.15, 0.0, 1, motor[3].valueSet.pulse);
	...
	for (int i = 0; i < 4; i++)
	{
		if(motorlimit.openzero == true)
		motor[i].enable = ENABLE;
		motor[i].status.clearFlag = 1;
...
void Zero_mode(s16 id)
{
	...
	if (motor[id].argum.zeroCnt > 30) //寻零完成，初始化零点，转换为位置模式锁零点
	{
		motor[id].argum.zeroCnt = 0;
		...
		{
			case 0:
		motor[0].valueReal.angle = -30.f;
			case 1:
		motor[1].valueReal.angle = -150.f;
			case 2:
		motor[2].valueReal.angle = -90.f;		
		}
		motor[id].valueReal.pulse = motor[id].valueReal.angle / 360.f * motor[id].intrinsic.RATIO * motor[id].intrinsic.GEARRATIO *motor[id].intrinsic.LINE; //输出轴
		motor[id].mode = position;
		motor[id].valueSet.angle = 0;
		motor[id].valueSet.pulse = 0;
		motor[id].enable = false;
	}
}
...
{
	motor[id].valueSet.pulse = motor[id].valueSet.angle* motor[id].intrinsic.GEARRATIO * motor[id].intrinsic.LINE* motor[id].intrinsic.RATIO/ 360.f;
...
void ifdjtimeout(u16 id)
{
	if (motor[id].enable == 1)
	{
		if (motor[id].limit.Motortimeout)
		{
			...
			if(motor[0].status.timeout == 0 && motor[1].status.timeout == 0 && motor[2].status.timeout == 0 )//&& motor[3].status.timeout == 0
			{
				Led8_show(0);
			}
		}
...
void SetM3508_1(short ID1, short ID2, short ID3, short ID4) //发送5-8电机
{
	...
	if (!ifdjstuck(0) && motor[0].enable == 1) 
	{
		tx_message.Data[0] = (ID1 & 0xFF00) >> 8;
		tx_message.Data[1] = ID1 & 0xFF; 
	}
	else
	{
		tx_message.Data[0] = 0;
...
void peakcurrent(void)
{
	...
	if(motor[id].mode ==  zero)
		PEAK(motor[id].valueSet.current,motor[id].limit.zeroCurrent);
	}
}

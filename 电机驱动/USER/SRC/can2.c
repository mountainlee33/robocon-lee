...
void CAN2_Mode_Init(void)
{
	...
	CAN_FilterInitStructure.CAN_FilterMaskIdLow = 0x00 << 5;
	CAN_FilterInitStructure.CAN_FilterFIFOAssignment = CAN_FilterFIFO1;
	CAN_FilterInitStructure.CAN_FilterActivation = ENABLE;
	CAN_FilterInit(&CAN_FilterInitStructure);
...
#endif
#ifdef USE_ODRIVE
		if (((rx_message.StdId >= 0x201) && (rx_message.StdId <= 0x208)) && (rx_message.RTR == CAN_RTR_Data)) //odrive反馈报文
		{//canopen通信协议PD0：0x200 + nodeID
			if ((rx_message.StdId | 0x1F) == 0x014) //IQ

			if ((rx_message.StdId | 0x1F) == 0x009)
			{
			}
		}

...
{
	AK80.RATIO = 6.0f;    			//内部减速器减速比为6：1
	AK80.GEAR_RATIO = 1.0f;			//外部减速器为1
	AK80.PULSE = 4096;					//脉冲数
...
void Enter_Motor_Control(u8 id,u8 cmd)
{
	...
	switch(cmd)
	{
		case CMD_MOTOR_MODE://进入控制模式
			initiation[7] = 0XFC;
			break;
		case CMD_RESET_MODE://退出控制模式
			initiation[7] = 0XFD;
			break;
		case CMD_ZERO_POSITION://设置当前位置为零点
			initiation[7] = 0XFE;
			break;
		default:return;
	}
...
u8 buf[8];
//实现纯位置、纯速度、纯转矩控制，仅需给对应的变量赋值其余量赋 0
void AK_Transmit(u8 id, float location, float vel, float f_kp, float f_kd, float tor)
{
	...
	f_kp = fminf(fmaxf(0, f_kp), 100);//p取值范围0-500
	f_kd = fminf(fmaxf(0, f_kd), 5);//d取值范围：0-5
	tor = fminf(fmaxf(tor, -18),18); //峰值扭矩18NM

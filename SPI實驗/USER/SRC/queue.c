...
void Can_DeQueue(CAN_TypeDef *CANx, Can_QueueTypeDef *can_queue)
{
	...
	{
		if ((can_queue->Can_DataSend[can_queue->Front].ID > 0x300)&(can_queue->Can_DataSend[can_queue->Front].ID < 0x310)) //elmo报文
		{
		...
		else if (can_queue->Can_DataSend[can_queue->Front].ID < 0x008) //AK报文
		{
		...
		else if ((can_queue->Can_DataSend[can_queue->Front].ID > 0x200)&&(can_queue->Can_DataSend[can_queue->Front].ID < 0x419)) //ODRIVE报文
			{
				...
				ttTxMessage.StdId = can_queue->Can_DataSend[can_queue->Front].ID; //标识符为设置的id号
				ttTxMessage.DLC = can_queue->Can_DataSend[can_queue->Front].DLC;
				if ((can_queue->Can_DataSend[can_queue->Front].ID == 0x203 )||(can_queue->Can_DataSend[can_queue->Front].ID == 0x204 )||(can_queue->Can_DataSend[can_queue->Front].ID == 0x209 )||(can_queue->Can_DataSend[can_queue->Front].ID == 0x214 )) 
					ttTxMessage.RTR = CAN_RTR_REMOTE;
				else if ((can_queue->Can_DataSend[can_queue->Front].ID == 0x403 )||(can_queue->Can_DataSend[can_queue->Front].ID == 0x404 )||(can_queue->Can_DataSend[can_queue->Front].ID == 0x409 )||(can_queue->Can_DataSend[can_queue->Front].ID == 0x414 )) 
					ttTxMessage.RTR = CAN_RTR_REMOTE;
				else

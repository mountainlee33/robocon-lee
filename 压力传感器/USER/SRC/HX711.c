 ...
 * @LastEditTime: 2021-04-04 22:17:00
...
void Init_HX711pin(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = presensor[1].pregpio.Pin_SCK;	   // 端口配置
	GPIO_Init(presensor[1].pregpio.GPIO_SCK, &GPIO_InitStructure); //根据设定参数初始化GPIOB
...
u32 HX711_Read(PRESENSOR *Presensor) //增益128
{
	...
	*(Presensor->pregpio.HX711_SCK) = 0; //给SCK输出低电平？
	count = 0;
	if (*(Presensor->pregpio.HX711_DOUT)==0)//读取到0开始进行计算
	{
	for (i = 0; i < 24; i++)
	{
	...
	*(Presensor->pregpio.HX711_SCK) = 0;
	...
	{
		Presensor->preflag.DOUT_error=1;
		LED_RED_OFF;
		Delay_ms(500);
	}
	return (count);
...
void Get_Peel(void)
{
	...
	{
	if (presensor[num].preflag.enable == 1)
	{
...
void Set_Zero(void)
{
	...
	for(num=0;num<SensorNum;num++)
	{
		if(presensor[num].preflag.SetZero == 1)
		{

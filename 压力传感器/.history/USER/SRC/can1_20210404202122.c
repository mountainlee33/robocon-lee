	...
	if (CAN_GetITStatus(CAN1, CAN_IT_FMP0) != RESET)
	{
		...
		CAN_Receive(CAN1, CAN_FIFO0, &rx_message);
			...
			if ((rx_message.Data[0] == 'p') && (rx_message.Data[1] == 'r')&&(rx_message.ExtId == ID_SELF))
			{
				...
				Can1_Send_Pressure(&presensor[0]);
			}
	}
}

void Can1_Send_Pressure(PRESENSOR *Presensor)
{
	CanTxMsg TXmessage;
	...
	TXmessage.Data[0] = 'p';
	TXmessage.Data[1] = 'r';
	TXmessage.Data[2] = Presensor->number;
	TXmessage.Data[3] = (uint8_t)((int16_t)(Presensor->pressure>>16)) ;
	TXmessage.Data[4] = (uint8_t)(((int16_t)(Presensor->pressure>>16))>>8) ;
	TXmessage.Data[5] = (uint8_t)((int16_t)(Presensor->pressure)) ;
	TXmessage.Data[6] = (uint8_t)(((int16_t)(Presensor->pressure))>>8);
